...
`.github/workflows/ci.yml` runs lint, typecheck, unit tests, the build, the changeset check and e2e, so a red gate surfaces either way - locally is just faster than a round trip.
...
```
feat(time-entries): add entry form
```
