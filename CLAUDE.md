...
**Status: US-4 (with X-1), the core stories complete.** Tooling and the API layer are complete; login, the session and the auth-guarded route tree are in place; the day view lists a selected date's entries with the week strip and totals around them; the entry form both creates one, against the service chosen in the settings sheet (A-1), and edits an existing one at `/entries/$id/edit`, keeping the entry's own service; and an entry can be deleted from either, behind one confirm dialog. Descriptions are rich text in both directions (A-9 as amended by ADR-0010). US-1 to US-4 close the assignment's own stories, so `v0.2.0` is cuttable; the extras (SPEC 10) are next, X-2 first.
...
Parts of the day view are **drawn but inert**, because the design puts them on this screen and a bar or card that gained a control later would reflow around it: the timer pill and the `?` sheet in the app bar, the quick-add line (it opens the form without parsing), the kebab's `Continue timer` and `Duplicate`, and `Copy from yesterday`. Each belongs to X-2, X-3 or X-4 and is wired there. `Default service...` was one of them and is now live (US-2), as is the kebab's `Edit` (US-3), and its `Delete` and the edit form's `Delete entry` (US-4).
